...
try {
    ...
    if ($certs.Count -gt 0) {
        ...
        Write-Host "ℹ️  El usuario no tiene certificaciones aún" -ForegroundColor Yellow
    }
} catch {
...
Write-Host ""
Write-Host "================================================" -ForegroundColor Cyan
Write-Host "  Test 2: POST Certificación (opcional)" -ForegroundColor Yellow
Write-Host "================================================" -ForegroundColor Cyan

$testCert = Read-Host "¿Quieres crear una certificación de prueba? (s/n)"
...
if ($testCert -eq "s") {
    ...
    try {
        ...
        Write-Host "✅ Certificación creada exitosamente" -ForegroundColor Green
        ...
        $deleteCert = Read-Host "¿Eliminar esta certificación de prueba? (s/n)"
        if ($deleteCert -eq "s") {
            try {
                Invoke-RestMethod -Uri "$API_URL/users/$userId/certifications/$($newCert.id)" `
                    -Method DELETE
                Write-Host "✅ Certificación eliminada" -ForegroundColor Green
            } catch {
                Write-Host "❌ Error al eliminar certificación" -ForegroundColor Red
            }
        }
    } catch {
        Write-Host "❌ ERROR al crear certificación" -ForegroundColor Red
        if ($_.Exception.Response) {
...
try {
    ...
    if ($exps.Count -gt 0) {
        ...
        Write-Host "ℹ️  El usuario no tiene experiencias aún" -ForegroundColor Yellow
    }
} catch {
...
if ($testExp -eq "s") {
    ...
    try {
        ...
        Write-Host "  - Trabajo actual: Sí" -ForegroundColor White
        Write-Host ""
...
Write-Host ""
Write-Host "================================================" -ForegroundColor Cyan
Write-Host "  Test 5: Upload de Certificación (opcional)" -ForegroundColor Yellow
Write-Host "================================================" -ForegroundColor Cyan
...
if ($testUpload -eq "s") {
    if ($script:certId) {
        ...
        $certIdForUpload = Read-Host "Ingresa el ID de una certificación"
    }
